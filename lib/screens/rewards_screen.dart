...
class RewardsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (reward == null) {
      return const AppCard(child: Text('لا توجد بيانات مكافآت بعد.'));
    }
    ...
    return Column(
      ...
      children: [
        ResponsiveGrid(
          children: [
            StatTile(
                label: 'XP', value: '${reward.xp}', icon: Icons.bolt_outlined),
            StatTile(
                label: 'المستوى',
                value: '${reward.level}',
            ...
            children: [
              ...
              if (!app.isParent)
                Wrap(
                  ...
                  children: [
                    FilledButton.icon(
                        onPressed: () => _addXp(app, reward, 10),
                        icon: const Icon(Icons.add),
                        label: const Text('إضافة 10 XP')),
                    FilledButton.tonalIcon(
                        onPressed: () => _addStreak(app, reward),
                        icon: const Icon(Icons.today),
                        label: const Text('زيادة السلسلة')),
                    FilledButton.tonalIcon(
                        onPressed: () => _addBadge(app, reward),
  ...
  Future<void> _addXp(AppProvider app, Reward reward, int value) {
    ...
    return app.saveReward(Reward(
        ...
        dailyStreak: reward.dailyStreak));
  }
  ...
  Future<void> _addStreak(AppProvider app, Reward reward) {
    return app.saveReward(Reward(
        ...
        level: reward.level,
        badges: reward.badges,
        dailyStreak: reward.dailyStreak + 1));
  }
